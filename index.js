(async()=>{
    ...
    // Main
    console.log("Grabbing installed packages, please wait...")
    var packages = shellJS.exec("npm list", { silent: true }).stdout
    ...
    await runJobs(
        ...
        async(p)=>{
            try{
                var response = await request(`https://socket.dev/api/npm/package-info/score?name=${p}`)
                ...
                result.push({ name: p, malicious: response.match("This is a malicious package") ? true : false, supplyChainRisk: supplyChainRisk, quality: quality, license: license })
            }catch{}
        },
        {
            concurrency: 50
